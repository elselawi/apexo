...
void main() {
  ...
  group("Save Remote", () {
    ...
    test('put with an empty batch is a no-op', () async {
      final before = await saveRemote.getVersion();
      expect(await saveRemote.put([]), isTrue);
      expect(await saveRemote.getVersion(), before);
    });
    ...
    test(
        "getting version should give an updated version number representing the date",
        () async {
    ...
    test("Each version should give different result", () async {
      ...
      expect(
          result0.rows.where((item) => item.data == '{"key":"value1"}').length,
          equals(1));
      expect(
          result0.rows.where((item) => item.data == '{"key":"value2"}').length,
          equals(1));
      expect(
          result0.rows.where((item) => item.data == '{"key":"value3"}').length,
          equals(1));

      expect(
          result1.rows.where((item) => item.data == '{"key":"value1"}').length,
          equals(0));
      expect(
          result1.rows.where((item) => item.data == '{"key":"value2"}').length,
          equals(1));
      expect(
          result1.rows.where((item) => item.data == '{"key":"value3"}').length,
          equals(1));

      expect(
          result2.rows.where((item) => item.data == '{"key":"value1"}').length,
          equals(0));
      expect(
          result2.rows.where((item) => item.data == '{"key":"value2"}').length,
          equals(0));
      expect(
          result2.rows.where((item) => item.data == '{"key":"value3"}').length,
          equals(1));

      expect(
          result3.rows.where((item) => item.data == '{"key":"value1"}').length,
          equals(0));
      expect(
          result3.rows.where((item) => item.data == '{"key":"value2"}').length,
          equals(0));
      expect(
          result3.rows.where((item) => item.data == '{"key":"value3"}').length,
          equals(0));
    ...
    test("uploading images", () async {
      // uploading
      await saveRemote.uploadImage(
          rowID: savedId,
          filename: "x_file1.txt",
          predefinedMultipart: MultipartFile.fromString("imgs+", "content",
              filename: "x_file1.txt"));
      ...
      expect(
          List<String>.from(record.data["imgs"])
              .where((item) => item.contains("file1"))
              .length,
          1);
    ...
    test('getImageLink returns an exact server-file URL', () async {
      final link = await saveRemote.getImageLink(savedId, 'file1.txt');

      expect(link, isNotNull);
      expect(link, startsWith('${pb.baseURL}/api/files/data/$savedId/'));
      expect(link, contains('file1'));
    });

    test('getImageLink returns null for a file absent from the server',
        () async {
      final link = await saveRemote.getImageLink(
        savedId,
        'does-not-exist.txt',
        false,
      );
    ...
    test("appending one more image", () async {
      ...
      expect(
          List<String>.from(record.data["imgs"])
              .where((item) => item.contains("file2"))
              .length,
          1);
    ...
    test(
        "trying to append multiple images, some of which is duplicated, others are not",
        () async {
      await saveRemote.uploadImage(
          rowID: savedId,
          filename: "x_file1.txt",
      ...
      await saveRemote.uploadImage(
          rowID: savedId,
          filename: "x_file3.txt",
          predefinedMultipart: MultipartFile.fromString("imgs+", "content",
              filename: "x_file3.txt"));
      await saveRemote.uploadImage(
          rowID: savedId,
          filename: "x_file4.txt",
          predefinedMultipart: MultipartFile.fromString("imgs+", "content",
              filename: "x_file4.txt"));
      final record = await pb.collection("data").getOne(savedId);
      ...
      expect(
          List<String>.from(record.data["imgs"])
              .where((item) => item.contains("file3"))
              .length,
          1);
      expect(
          List<String>.from(record.data["imgs"])
              .where((item) => item.contains("file4"))
              .length,
          1);
    });
    test("trying to append multiple images, all of which are duplicated",
        () async {
      await saveRemote.uploadImage(
          rowID: savedId,
          filename: "x_file1.txt",
    ...
    test("trying to append multiple images, none of which are duplicated",
        () async {
      await saveRemote.uploadImage(
          rowID: savedId,
          filename: "x_file5.txt",
          predefinedMultipart: MultipartFile.fromString("imgs+", "content",
              filename: "x_file5.txt"));
      await saveRemote.uploadImage(
          rowID: savedId,
          filename: "x_file6.txt",
          predefinedMultipart: MultipartFile.fromString("imgs+", "content",
              filename: "x_file6.txt"));
      await saveRemote.uploadImage(
          rowID: savedId,
          filename: "x_file7.txt",
          predefinedMultipart: MultipartFile.fromString("imgs+", "content",
              filename: "x_file7.txt"));
      await saveRemote.uploadImage(
          rowID: savedId,
          filename: "x_file8.txt",
          predefinedMultipart: MultipartFile.fromString("imgs+", "content",
              filename: "x_file8.txt"));
      final record = await pb.collection("data").getOne(savedId);
      ...
      expect(
          List<String>.from(record.data["imgs"])
              .where((item) => item.contains("file5"))
              .length,
          1);
      expect(
          List<String>.from(record.data["imgs"])
              .where((item) => item.contains("file6"))
              .length,
          1);
      expect(
          List<String>.from(record.data["imgs"])
              .where((item) => item.contains("file7"))
              .length,
          1);
      expect(
          List<String>.from(record.data["imgs"])
              .where((item) => item.contains("file8"))
              .length,
          1);
    });
